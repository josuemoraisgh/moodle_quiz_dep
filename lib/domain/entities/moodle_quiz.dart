...
class MoodleQuiz extends Equatable {
  ...
  final String
      preferredBehaviour; // ex: 'immediatefeedback', 'deferredfeedback'
  ...
  List<String> get incompatibilityReasons {
    ...
    if (!isImmediateFeedback) {
      reasons.add(
          'Comportamento deve ser "Feedback imediato" (atual: $preferredBehaviour)');
    }
    if (!showsCorrectnessOnAttempt) {
      reasons.add(
          'Opções de revisão → "Se está correto" deve estar marcado durante tentativa');
    }
    return reasons;
  }
